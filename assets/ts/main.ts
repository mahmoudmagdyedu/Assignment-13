...
document.getElementById('add-btn')?.addEventListener('click', (): void => {
    updateIndex = -1;
    let modalTitle = document.querySelector('#pop p');
    if (modalTitle) modalTitle.textContent = 'Create New Task';
    let subBtn = document.getElementById('sub-btn');
    if (subBtn) subBtn.innerHTML = '<i class="fa-solid fa-plus text-xs"></i> Add Task';
    let tname = document.querySelector<HTMLInputElement>('[placeholder="What needs to be done?"]');
    let preyorty = document.querySelector<HTMLInputElement>('#preyorty');
    let date = document.querySelector<HTMLInputElement>('#date');
    let descr = document.querySelector<HTMLTextAreaElement>('[placeholder="Add more details about this task..."]');
    clearend(tname, preyorty, date, descr);
    popup?.classList.remove('hidden');
});

document.getElementById('exit')?.addEventListener('click', (): void => {
...
document.getElementById('cancel-btn')?.addEventListener('click', (): void => {
    popup?.classList.add('hidden');
    let tname = document.querySelector<HTMLInputElement>('[placeholder="What needs to be done?"]');
    let preyorty = document.querySelector<HTMLInputElement>('#preyorty');
    let date = document.querySelector<HTMLInputElement>('#date');
...
let titleInput = document.querySelector<HTMLInputElement>('[placeholder="What needs to be done?"]');
...
let dateInp = document.querySelector<HTMLInputElement>('#date');
dateInp?.addEventListener('input', (): void => {
    dateInp?.classList.remove('border-red-500');
    dateInp?.classList.add('border-gray-200');
    document.getElementById('date-error')?.classList.add('hidden');
});

let descInput = document.querySelector<HTMLTextAreaElement>('[placeholder="Add more details about this task..."]');
descInput?.addEventListener('input', (): void => {
...
class Task {
    ...
    state: string;
    time: number;
    constructor(title: string, pre: string, date: string, desc?: string, state: string = 'to-do', time: number = Date.now()) {
        this.title = title;
        this.pre = pre;
        this.date = date;
        this.desc = desc;
...
function clearend(tname: HTMLInputElement | null, preyorty: HTMLInputElement | null, date: HTMLInputElement | null, descr: HTMLTextAreaElement | null): void {
    if (tname) {
        tname.value = '';
        tname.classList.remove('border-red-500');
        tname.classList.add('border-gray-200');
    }
    if (preyorty) preyorty.value = 'Medium';
    if (date) {
        date.value = '';
        date.classList.remove('border-red-500');
        date.classList.add('border-gray-200');
...
let cardD: Task[] = JSON.parse(localStorage.getItem('tasks') || '[]');

function gentask(cont: Task[]): void {
    let boxTodo: string = '';
    let boxPro: string = '';
    let boxComp: string = '';
    ...
    if (cardToDo) cardToDo.innerHTML = boxTodo;
    let todoCount = document.getElementById('todo-count');
    if (todoCount) todoCount.innerText = `${countTodo} tasks`;
    let noFile = document.getElementById('no-file');
    if (noFile) {
        if (countTodo === 0) noFile.classList.remove('hidden');
        else noFile.classList.add('hidden');
    }

    const proCard = document.getElementById('pro-card');
    if (proCard) proCard.innerHTML = boxPro;
    let proCount = document.getElementById('pro-count');
    if (proCount) proCount.innerText = `${countPro} tasks`;
    let noFileProgress = document.getElementById('no-file-progress');
    if (noFileProgress) {
        if (countPro === 0) noFileProgress.classList.remove('hidden');
        else noFileProgress.classList.add('hidden');
    }

    const compCard = document.getElementById('comp-card');
    if (compCard) compCard.innerHTML = boxComp;
    let compCount = document.getElementById('comp-count');
    if (compCount) compCount.innerText = `${countComp} tasks`;
    let noFileCompleted = document.getElementById('no-file-completed');
    if (noFileCompleted) {
        if (countComp === 0) noFileCompleted.classList.remove('hidden');
        else noFileCompleted.classList.add('hidden');
    }
}

function changeState(index: number, newState: string): void {
    let tasks: Task[] = JSON.parse(localStorage.getItem('tasks') || '[]');
    if (tasks[index]) {
...
function editTask(index: number): void {
    ...
    if (tname) tname.value = tasks[index].title || '';
    if (preyorty) preyorty.value = tasks[index].pre || 'Medium';
    if (date) date.value = tasks[index].date || '';
    if (descr) {
        descr.value = tasks[index].desc || '';
        let charCount = document.getElementById('char-count');
        if (charCount) charCount.innerText = `${descr.value.length}/500`;
    }

    let modalTitle = document.querySelector('#pop p');
    if (modalTitle) modalTitle.textContent = 'Update Task';
    let subBtn = document.getElementById('sub-btn');
    if (subBtn) subBtn.innerHTML = '<i class="fa-solid fa-check text-xs"></i> Update Task';

    popup?.classList.remove('hidden');
}

(window as any).changeState = changeState;
